...
Future<List<TotalDepositoAhorroStruct>> getTotalForAccount(
    List<String> ahorrosId) async {
  if (ahorrosId.isEmpty) {
    return [];
  }
  ...
  try {
    ...
    // Firestore permite hasta 10 IDs por "whereIn", así que dividimos en chunks
    Map<String, double> totalesPorId = {};

    // Inicializar todos los IDs con 0
    for (String id in ahorrosId) {
      totalesPorId[id] = 0.0;
    }

    // Dividir en chunks de 10 (límite de Firestore para "whereIn")
    const chunkSize = 10;
    for (int i = 0; i < ahorrosId.length; i += chunkSize) {
      final chunk = ahorrosId.sublist(
        i,
        i + chunkSize > ahorrosId.length ? ahorrosId.length : i + chunkSize,
      );
    ...
    List<TotalDepositoAhorroStruct> resultados = [];
    for (String id in ahorrosId) {
      resultados.add(TotalDepositoAhorroStruct(
    ...
    return ahorrosId
        .map((id) => TotalDepositoAhorroStruct(
              total: 0.0,
              ahorroid: id,
            ))
        .toList();
