...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Ha pasado mucho tiempo desde su última actividad. Debe realizar el login nuevamente.')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  '¡Correo o contraseña incorrecta! Por favor, volver a intentarlo')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              'Se envió el correo para restaurar su contraseña. Verifique su bandeja de entrada.')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : AlianzaFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'El correo ingresado ya existe.',
        'INVALID_LOGIN_CREDENTIALS' =>
          'Los datos ingresados son incorrectos o no coinciden. Veriique y vuelva a intentar.',
        _ => '¡Correo o contraseña incorrecta! Por favor, volver a intentarlo',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
